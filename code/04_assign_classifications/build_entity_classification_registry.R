...
# in assign_final_classification.Rmd (requires entity_registry_approved.csv in 04_inputs/).
#
# Three linking tiers:
#   1. Names 2026 mapping (name_entity_mapping_rebuilt.csv) 
#   2. DD org concept map (dd_names_mapped_to_org_concepts_3_18_2026.csv) 
#   3. name_v5 exact match (business suffixes stripped)
...
nem_raw <- read_csv(
  "../01_contributor_pipeline/01_inputs/name_entity_mapping_rebuilt.csv",
...
dd_raw <- read.csv(
  "../01_contributor_pipeline/01_inputs/dd_names_mapped_to_org_concepts_3_18_2026.csv",
...
# ── Step 2: Three-tier match generation ────────────────────────────────

# Tier 1: Names 2026 mapping
...
reg_path <- sprintf("04_outputs/entity_classification_registry_%s.csv", today)
...
# Entities with code conflicts 
conflicts_path <- sprintf("04_outputs/entity_classification_conflicts_%s.csv", today)
write_csv(
  registry_final |>
...
# (gov/public_safety, from manual_review) but rule_code was "health"
overrides_path <- sprintf("04_outputs/entity_pipeline_overrides_%s.csv", today)
write_csv(
  registry_final |>
...
lookup_path <- sprintf("04_outputs/entity_name_lookup_%s.csv", today)
write_csv(
  all_name_comps |>
