...
produced by the 01 pipiline, translating all NAICS codes to the
current custom sector scheme. Pass the output to 0201_train_classifier.py
via --train.

Sources:
  C  01_inputs/already_classified_contributions.csv
...
Then retrain (no changes to 0201):
    python code/02_ml_classifier/0201_train_classifier.py \\
...
DEFAULT_PRE_CLASSIFIED = CODE_ROOT / "01_contributor_pipeline" / "01_inputs" / "already_classified_contributions.csv"
...
# Load normalize_name from 0105 via importlib (digit-prefix module) ---------------

_spec = importlib.util.spec_from_file_location(
    "classify_contributors_0105",
    CODE_ROOT / "01_contributor_pipeline" / "0105_classify_contributors.py",
)
_m0105 = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = _m0105
_spec.loader.exec_module(_m0105)

normalize_name = _m0105.normalize_name   # pd.Series -> pd.Series
...
#   "*keyword*"   keyword match (0105 style)
#   "*identity*"  identity-based override (0105 style)
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"\nWrote: {args.out}")
    print("\nNext step:")
    print(f"  python code/02_ml_classifier/0201_train_classifier.py \\")
