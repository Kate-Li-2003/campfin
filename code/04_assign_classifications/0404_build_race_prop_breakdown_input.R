...
# we're tracking, with a NAICS code attached when available, plus a
# `candidate` column (Recipient Name, falls back to race_prop for ballot
# measures) -- compute_breakdown.R breaks the
...
# code per contribution_id (preferring a real code over blank/"99")
...
POWER_SEARCH_PATH   <- "../01_contributor_pipeline/01_inputs/power_search_contributions_normalized_08-20-26_updated_entity.csv"
CLASSIFICATION_INPUT_PATH <- "../01_contributor_pipeline/01_outputs/classification_input_combined.csv"
#FINAL_CLASSIFICATIONS_PATH <- "04_outputs/final_classifications_before_review.csv"
FINAL_CLASSIFICATIONS_PATH <- "04_outputs/final_classifications_08-23-25.csv"
PAC_INPUT_PATH       <- "../01_contributor_pipeline/01_outputs/pac_input.csv"
PAC_INDUSTRY_BREAKDOWN_PATH <- "04_outputs/pac_industry_breakdown.csv"
...
# so pac_id_key = NA and the ID-keyed join below won't work for them. Fall back to
# matching by name 
# Only entries with a real industry code (not "99") are included — in
# match_pac_classifications, federal/candidate PACs get sector code,
# while non-federal no-ID PACs get "99"
...
# PACs not yet in pac_review) is left as  uncategorized remainder
pac_split_rows <- pac_input %>%
  filter(!is.na(pac_id_key) & trimws(coalesce(pac_id_key, "")) != "") %>%
  left_join(
    pac_industry_breakdown %>% select(pac_id, industry, pct_of_total),
    by = c("pac_id_key" = "pac_id"),
    relationship = "many-to-many"  # one PAC contribution explodes into N industry rows
  ) %>%
  mutate(
...
AUDIT_PATH <- paste0("04_outputs/race_prop_contributor_audit_", today, ".csv")
...
dir.create("04_outputs", showWarnings = FALSE)
