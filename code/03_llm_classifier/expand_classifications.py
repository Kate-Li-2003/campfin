...
0301_web_search.py dedupes classification_input.csv down to unique search_key
...
0301_web_search.py uses to compute search_key in the first place. The result
...
Usage
-----
    python expand_classifications.py --llm 03_outputs/classification_full_full_2026-08-13.csv \
                                      --llm 03_outputs/classification_full_web_cache_full_2026-08-13.csv \
                                      --out 03_outputs/classification_full_expanded_2026-08-13.csv
...
def compute_search_keys(classification_input_path: str = DEFAULT_INPUT_PATH) -> pd.DataFrame:
    ...
    same recipe 0301_web_search.py uses -- and return just [uuid, search_key]
