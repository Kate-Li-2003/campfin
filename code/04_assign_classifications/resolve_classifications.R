...
#
# Logic for picking one "best" NAICS code per contribution `uuid`, given up to four independent
# sources -- rule-based (0105_classify_contributors.py), OpenSecrets match, ML classification, 
# and LLM web-search classification. 
...
# produce their own naics code independently of 0105_classify_contributors.py's
# rule-based path -- the only path that runs codes through 0105's
...
# 0401_build_os_llm_naics_crosswalk.py whenever new OS/LLM output files are added.
OS_LLM_CROSSWALK_PATH <- "04_outputs/os_llm_naics_crosswalk.csv"

apply_os_llm_crosswalk <- function(code_vec, crosswalk_path = OS_LLM_CROSSWALK_PATH) {
  if (!file.exists(crosswalk_path)) {
    warning(sprintf(
      "os/llm NAICS crosswalk not found at %s -- codes used as-is. Regenerate with 0401_build_os_llm_naics_crosswalk.py.",
      crosswalk_path
    ))
    return(code_vec)
  }
